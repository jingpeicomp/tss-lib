...
import (
	...
	"github.com/bnb-chain/tss-lib/v2/gg20/keygen"
	"github.com/bnb-chain/tss-lib/v2/gg20/signing"
...
func SignOneRound() {
	...
	p2PCtx := tss.NewPeerContext(partyIDs)
	committees := make([]*signing.LocalParty, 0, len(partyIDs))
	params := make([]*tss.Parameters, 0, len(partyIDs))
	outCh := make(chan tss.Message, len(partyIDs))
	endChs := [2]chan *common.SignatureData{make(chan *common.SignatureData), make(chan *common.SignatureData)}
	msg := &big.Int{}
	msg.SetBytes([]byte("Hello web3 world!"))

	for i, pID := range partyIDs {
		param := tss.NewParameters(tss.EC(), p2PCtx, pID, len(partyIDs), 1)
		params = append(params, param)
		key := loadKey(i)
		party := signing.NewLocalPartyWithOneRoundSign(param, key, outCh, endChs[i]).(*signing.LocalParty)
		//party := signing.NewLocalParty(msg, params, key, outCh, endCh).(*signing.LocalParty)
		committees = append(committees, party)
	}
	...
	var signEnded int32
	signDataArray := make([]*common.SignatureData, len(committees))
	siArray := make([]*big.Int, len(committees))
	siMap := make(map[*tss.PartyID]*big.Int)
signing:
	for {
		log.Printf("ACTIVE GOROUTINES: %d\n", runtime.NumGoroutine())
		select {
		...
		case signData := <-endChs[0]:
			log.Println("Party 1 sign finish !")
			signDataArray[0] = signData
			siArray[0] = signing.FinalizeGetOurSigShare(signData, msg, params[0])
			siMap[partyIDs[0]] = siArray[0]
		...
		case signData := <-endChs[1]:
			log.Println("Party 2 sign finish !")
			signDataArray[1] = signData
			siArray[1] = signing.FinalizeGetOurSigShare(signData, msg, params[1])
			siMap[partyIDs[1]] = siArray[1]
	...
	pubKeyStr := "5dca48a5725a0c0e674124310ab259329d95695ae90ec92637bd715ac7f07037571060b65c7ff89d3eb5a44bb4dca20490eca0243296202c5c324a538d89db73"
	pubKey := parsePubKey(pubKeyStr)
	signing.FinalizeGetAndVerifyFinalSig(signDataArray[0], pubKey, msg, partyIDs[0], siArray[0], siMap, params[0])
	log.Println("GetSignatureRecovery = ", hex.EncodeToString(signDataArray[0].GetSignatureRecovery()))
	log.Println("S = ", hex.EncodeToString(signDataArray[0].GetS()))
	log.Println("R = ", hex.EncodeToString(signDataArray[0].GetR()))

	signing.FinalizeGetAndVerifyFinalSig(signDataArray[1], pubKey, msg, partyIDs[1], siArray[1], siMap, params[1])
	log.Println("GetSignatureRecovery = ", hex.EncodeToString(signDataArray[1].GetSignatureRecovery()))
	log.Println("S = ", hex.EncodeToString(signDataArray[1].GetS()))
	log.Println("R = ", hex.EncodeToString(signDataArray[1].GetR()))
}
...
func partyUpdate(party tss.Party, msg tss.Message) {
	...
	log.Printf("------------> Party update msg from party %v", party.PartyID().Id)
...
func parsePubKey(pubStr string) *ecdsa.PublicKey {
	pubBytes, _ := hex.DecodeString(pubStr)
	x, y := elliptic.Unmarshal(tss.S256(), pubBytes)
	return &ecdsa.PublicKey{Curve: tss.S256(), X: x, Y: y}
}
