...
func NewPDLwSlackProof(wit PDLwSlackWitness, st PDLwSlackStatement) PDLwSlackProof {
	...
	alpha := common.GetRandomPositiveInt(rand.Reader, q3)
	nSubOne := new(big.Int).Add(st.PK.N, one)
	beta := new(big.Int).Add(one, common.GetRandomPositiveInt(rand.Reader, nSubOne))
	rho := common.GetRandomPositiveInt(rand.Reader, qNTilde)
	gamma := common.GetRandomPositiveInt(rand.Reader, q3NTilde)
...
func (p PDLwSlackProof) Marshal() ([][]byte, error) {
	cb := cmts.NewBuilder()
	cb = cb.AddPart([]*big.Int{p.Z})
	cb = cb.AddPart([]*big.Int{p.U1.X(), p.U1.Y(), p.U2, p.U3})
	cb = cb.AddPart([]*big.Int{p.S1, p.S2, p.S3})
	ints, err := cb.Secrets()
	if err != nil {
		return nil, err
	}
