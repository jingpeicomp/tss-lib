...
func (round *round7) Start() *tss.Error {
	...
	outer:
		for j, msg := range round.temp.signRound6Messages {
			...
			gammaJG := crypto.ScalarBaseMult(ec, gammaJ)
	...
	bigR := crypto.NewECPointNoCurveCheck(ec, bigRX, bigRY)
	...
	for j, msg := range round.temp.signRound6Messages {
		...
		TI, err := r3msg.UnmarshalTI(ec)
		if err != nil {
			return round.WrapError(err, Pj)
		}
		bigSI, err := r6msg.UnmarshalSI(ec)
		if err != nil {
			return round.WrapError(err, Pj)
		}
		...
		// ZK STProof check
		stProof, err := r6msg.UnmarshalSTProof(ec)
		if err != nil {
			return round.WrapError(err, Pj)
		}
		if ok := stProof.Verify(ec, bigSI, TI, bigR, h); !ok {
	...
	sI := FinalizeGetOurSigShare(round.data, round.temp.m, round.Params())
	round.temp.sI = sI
