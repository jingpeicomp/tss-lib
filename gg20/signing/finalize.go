...
func FinalizeGetOurSigShare(state *common.SignatureData, msg *big.Int, roundParam *tss.Parameters) (sI *big.Int) {
	data := state.GetOneRoundData()

	N := roundParam.EC().Params().N
...
func FinalizeGetAndVerifyFinalSig(
	...
	roundParam *tss.Parameters,
) (*common.SignatureData, *btcecdsa.Signature, *tss.Error) {
	if len(otherSIs) == 0 {
	...
	bigR, err := crypto.NewECPoint(roundParam.EC(),
	...
	for Pj, sJ := range otherSIs {
		...
		bigRBarJ, err := crypto.NewECPoint(roundParam.EC(),
		...
		bigSI, err := crypto.NewECPoint(roundParam.EC(),
	...
	modNs := secp256k1.ModNScalar{}
	modNs.SetByteSlice(s.Bytes())
	modNr := secp256k1.ModNScalar{}
	modNr.SetByteSlice(r.Bytes())
	btcecdsa.NewSignature(&modNr, &modNs)
	return state, btcecdsa.NewSignature(&modNr, &modNs), nil
}
...
func (round *finalization) Start() *tss.Error {
	...
	data, _, err := FinalizeGetAndVerifyFinalSig(round.data, pk, round.temp.m, round.PartyID(), ourSI, otherSIs, round.Params())
	if err != nil {
