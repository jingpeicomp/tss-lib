...
func (round *round1) Start() *tss.Error {
	...
	kI := common.GetRandomPositiveInt(round.Rand(), round.EC().Params().N)
	gammaI := common.GetRandomPositiveInt(round.Rand(), round.EC().Params().N)
	...
	gammaIG := crypto.ScalarBaseMult(round.EC().Params(), gammaI)
	round.temp.gammaIG = gammaIG

	cmt := commitments.NewHashCommitment(round.Rand(), gammaIG.X(), gammaIG.Y())
	round.temp.deCommit = cmt.D

	// MtA round 1
	paiPK := round.key.PaillierPKs[i]
	cA, rA, err := paiPK.EncryptAndReturnRandomness(round.Rand(), kI)
	if err != nil {
	...
	for j, Pj := range round.Parties().IDs() {
		...
		pi, err := mta.AliceInitGg20(round.Params().EC(), paiPK, kI, cA, rA, round.key.NTildej[j], round.key.H1j[j], round.key.H2j[j], round.Rand())
		if err != nil {
