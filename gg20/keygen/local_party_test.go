...
func TestBadMessageCulprits(t *testing.T) {
	...
	p2pCtx := tss.NewPeerContext(pIDs)
	params := tss.NewParameters(tss.EC(), p2pCtx, pIDs[0], len(pIDs), 1)
	...
	assert.Equal(t,
		"task gg20-keygen, party {0,P[1]}, round 1, culprits [{1,2}]: message failed ValidateBasic: Type: binance.tss-lib.ecdsa-gg20.keygen.KGRound1Message, From: {1,2}",
		err2.Error())
}
	...
	for {
		...
		case save := <-endCh:
			...
			if atomic.LoadInt32(&ended) == int32(len(pIDs)) {
				...
				t.Log("GG20 ECDSA signing test done.")
